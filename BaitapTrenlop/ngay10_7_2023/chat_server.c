...
char clientsName[MAX_USER][100];
char PROTOCOLVA[7][5] ={"JOIN","MSG","PMSG","OP","KICK","TOPIC","QUIT"};
int numberOfClients = 0;
pthread_mutex_t numberOfClients_mutex = PTHREAD_MUTEX_INITIALIZER;
time_t t;
struct tm tm;
int check_val_nick_name(char *str){
...
int join(void *arg,char *buff){
    int client = *(int*)arg;
    if(check_cmd(buff)>2){
        char *message = "999 UNKNOW ERROR\n";
        send(client, message, strlen(message), 0);
        return 999;
    }
    char cmd[32], client_id[32], tmp[32];
    int ret = sscanf(buff, "%s%s%s", cmd, client_id, tmp);
    for(int i =0;i<numberOfClients;i++){
        if(clientsName[i]==client_id){
            char *message = "200 NICKNAME IN USE\n";
            send(client, message, strlen(message), 0);
            return 200;
        }
    }
    if(check_val_nick_name(client_id)==-1){
        char *message = "201 INVALID NICKNAME\n";
        send(client, message, strlen(message), 0);
        return 201;
    }
    char *message = "100 OK\n";
    ...
    for(int i =0;i<MAX_USER;i++){
        if(strchr(clientsName[i],'/')!=NULL){
            strcpy(clientsName,client_id);
        }
    }
    return 100;
}
