...
int find_str(char* context,char* substring){
    char *temp2 = NULL;
    char *tmp;
    tmp = malloc(255*sizeof(char));
    strcpy(tmp,context);
    int iCount = 0;
    while ( (temp2 = strstr(tmp, substring)) ) {
        ++iCount;
        tmp = temp2 + 1;
    }
    return iCount;
}
int find_str_link(char* context1,char* context2, char* substring){
    char *temp2 = NULL;
    char *tmp;
    tmp = malloc(510*sizeof(char));
    strcpy(tmp,context1);
    strcat(tmp,context2);
    int iCount = 0;
    while ( (temp2 = strstr(tmp, substring)) ) {
...
int main(int argc, char *argv[])
{
    ...
    int n;
    char length[255];
    read(client_socket,length,255);
    n = atoi(length);
    printf("length of context: %d\n",n);
    int received = 0;
    int first_receiving = 1;
    int time_showing_up = 0;
    char pre_storage_buffer[255];
    printf("total: %d packet\n",(int)(floor(n/255)+1));
    while(received<(int)(floor(n/255)+1)){
        if(first_receiving==1){
            bzero(storage_buffer,255);
            n = read(client_socket,storage_buffer,255);
            if(n<0){
                error("error on reading");
            }
            printf("Receiving packet %d, length: %ld...............\n",received,strlen(storage_buffer));
            time_showing_up = time_showing_up+find_str(storage_buffer,"0123456789");
            received++;
            first_receiving=0;
        }
        else{
            strncpy(pre_storage_buffer,storage_buffer,255);
            bzero(storage_buffer,255);
            n = read(client_socket,storage_buffer,255);
            if(n<0){
                error("error on reading");
            ...
            printf("Receiving packet %d, length: %ld...............\n",received,strlen(storage_buffer));
            time_showing_up = time_showing_up-find_str(pre_storage_buffer,"0123456789")+find_str_link(pre_storage_buffer,storage_buffer,"0123456789");
            received++;
        }

    }
    printf("Totally, Found %d substring in file that equals to '0123456789'\n",time_showing_up);
